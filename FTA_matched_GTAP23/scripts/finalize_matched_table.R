## ============================================================================
## Build the matched-FTA summary table from whichever stages are complete.
## Re-run after each stage; it auto-detects the stage result CSVs present.
## ============================================================================
...
results_dir <- "C:/Claude Code Project Folder/Fontange 2022/R_Replication/FTA_matched_GTAP23/results"
...
stage_meta <- tribble(
  ...
  "s2",   "results_stage2_etwfe.csv",       "(2) ETWFE",                "Yes", "Yes",   "No",
...
write_csv(sc, file.path(results_dir, "COMBINED_matched_comparison.csv"))
...
tbl <- body |> gt(groupname_col="panel", rowname_col="rowlab") |>
  tab_header(title = md("**The effect of *matched* FTAs on trade, by GTAP-23 sector**"),
             subtitle = md("*PPML gravity (HS6 pooled to GTAP-23); matched-FTA treatment, other-FTA control; dep. var. = bilateral trade*")) |>
...
gtsave(tbl, file.path(results_dir, "Table_matched_FTA_by_GTAP23.html"))
tryCatch(writeLines(as.character(as_latex(tbl)), file.path(results_dir, "Table_matched_FTA_by_GTAP23.tex")),
...
cat("\nOutputs: COMBINED_matched_comparison.csv, Table_matched_FTA_by_GTAP23.html/.tex\n")
